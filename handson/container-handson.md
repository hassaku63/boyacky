...
### 2.1 コードのダウンロード
[ハッサク(笑)](https://github.com/hassaku63/boyacky) の GitHub からコードをダウンロードします。
...
オプション `-it`vを付けてシェルを起動すると、あたかもコンテナ内に SSH でログインしたかのようにコンテナ内部を操作可能です。
...
実際の運用ではタグのイミュータビリティを`有効`にして、イメージの更新ごとに一意のタグを付けることをご検討します。
...
* HTML 内の入力用テキストボックスで文字数を 128 文字に制限する
    * input タグに maxlength 属性を設定して 128 文字までにする
* app.py の create_data 関数内で 128 文字より長い場合はカットする
    * boyaki[:128]
